...
}`),a.uniforms.add(new ui("stipplePatternTexture",s=>s.stippleTexture),new Y("stipplePatternSDFNormalizer",s=>Sr(s.stipplePattern)),new Y("stipplePatternPixelSizeInv",s=>1/wt(s))),e.stippleOffColorEnabled&&a.uniforms.add(new Be("stippleOffColor",s=>mr(s.stippleOffColor))),a.code.add(p`float getStippleSDF(out bool isClamped) {
...
return isClamped ? floor(antiAliasedResult + 0.5) : antiAliasedResult;
  ...
  `)}function Sr(t){return t?(Math.floor(.5*(fr(t)-1))+.5)/t.pixelRatio:1}function wt(t){const e=t.stipplePattern;return e?hr(t.stipplePattern)/e.pixelRatio:1}const _r=.4,Nt=64,Tr=Nt/2,br=Tr/5,Or=Nt/br,Ea=.25;function yr(t,e){const i=t.vertex;Xe(i),i.uniforms.get("markerScale")==null&&i.constants.add("markerScale","float",1),i.constants.add("markerSizePerLineWidth","float",Or).code.add(p`float getLineWidth() {
...
vec2(vStippleDistanceLimits.x, 1e34);`)),a.main.add(p`
    ...
    }`),e.fragment.include(Ti,t),e.include(bi,t),s.include(Oi),s.main.add(p`discardBySlice(vpos);
